                ...
                if (resp.statusCode == 200) {
                    ...
                    $('body').children().each((i, e) => {
                        ...
                        if (eleHTML.indexOf('window._sharedData') > -1) {
                            resolve(JSON.parse(eleHTML.substring(21).slice(0, -1)).entry_data.ProfilePage[0].graphql.user);
                            return false;
                        }
                    })
                } else
            ...
            if (!err) {
                if (resp.statusCode == 200) {
                    ...
                    $('body').children().each((i, e) => {
                        eleHTML = $(e).html();
                        if (eleHTML.indexOf('window._sharedData') > -1) {
                            resolve(JSON.parse(eleHTML.substring(21).slice(0, -1)).entry_data.ProfilePage[0].graphql.user.id);
                            return false;
                        }
                    })
                } else
            ...
            if (!err) {
                if (resp.statusCode == 200) {
                    ...
                    $('body').children().each((i, e) => {
                        eleHTML = $(e).html()
                        if (eleHTML.indexOf('window._sharedData') > -1) {
                            resolve(JSON.parse(eleHTML.substring(21).slice(0, -1)).entry_data.ProfilePage[0].graphql.user.profile_pic_url_hd);
                            return false;
                        }
                    })
                } else
